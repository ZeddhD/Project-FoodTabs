      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          ...
          <div className="empty-state__desc">
            {filterStatus === 'all'
              ...
              : 'Switch to "All" to see your full booking history.'}
          </div>
          {filterStatus === 'all' && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {filtered.map(booking => {
            ...
            const hasDeposit = booking.restaurantId?.bookingDeposit > 0;
            ...
            return (
              <div key={booking._id} style={{
                ...
                <div style={{ padding: '16px 20px', display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                  <div>
                    ...
                    <div style={{ fontWeight: 600, fontSize: 13 }}>
                      {dateStr}{booking.time ? ` · ${booking.time}` : ''}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--clr-text-muted)', fontWeight: 600, marginBottom: 3 }}>Party Size</div>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>
                  ...
                  {booking.tablePreference && (
                    <div>
                      <div style={{ fontSize: 11, color: 'var(--clr-text-muted)', fontWeight: 600, marginBottom: 3 }}>Table</div>
                  ...
                  {hasDeposit && (
                    <div>
                      <div style={{ fontSize: 11, color: 'var(--clr-text-muted)', fontWeight: 600, marginBottom: 3 }}>Deposit</div>
                      <span style={{
                        padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700,
                        background: isPaid ? '#DCFCE7' : '#FEF9C3',
                        color:      isPaid ? '#16A34A' : '#92400E',
                      }}>
                        {isPaid ? '✓ Paid' : `৳${booking.restaurantId.bookingDeposit.toLocaleString()} due`}
                      </span>
                    </div>
                  )}
                </div>
                ...
                {/* Actions */}
                <div style={{
                  padding: '12px 20px', borderTop: '1px solid var(--clr-border)',
                  display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap'
                }}>
                  {/* Deposit payment CTA */}
                  {hasDeposit && !isPaid && canCancel && (
                    <Link
                      to={`/payment/${booking._id}`}
                      className="btn btn-primary btn-sm"
                    >
                      Pay ৳{booking.restaurantId.bookingDeposit.toLocaleString()} deposit
                    </Link>
                  )}

                  {/* Write a review after visit is completed */}
                  {isCompleted && booking.restaurantId?._id && (
                    <Link
                      to={`/review/create?restaurantId=${booking.restaurantId._id}`}
                      className="btn btn-primary btn-sm"
                    >
                      Write a Review
                    </Link>
